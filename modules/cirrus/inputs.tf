...
variable "cirrus_feeder_definitions_variables" {
  description = <<-DESCRIPTION
  (Optional) Map of maps to strings used when templating feeder YAML definitions prior to their conversion to HCL. Intended for abstracting environment-specific values away from the feeder definition. One such example of this would be targeting different ECR image tags in a `dev` and `prod` environment.

  The suggested (but not required) structure of this map is to group template variables by their feeder name:
  ```
  {
    example-feeder-1 = {
      image_tag = "v1.0"
  ...
  Your feeder YAML definitions would leverage this templating by using an attribute lookup in an interpolation sequence (`$${...}`) like so:
  ```
  name: example-feeder-2
  common_role_statements:
    - sid: ReadSomeBucketThatChangesForEachEnvironment
      ...
      resources:
        - arn:aws:s3:::\$${example-feeder-2.data_bucket}
        - arn:aws:s3:::\$${example-feeder-2.data_bucket}/*
  lambda:
    ecr_image_uri: your-ECR-URI:\$${example-feeder-2.image_tag}
...
variable "cirrus_feeder_definitions_variables_ssm" {
  description = <<-DESCRIPTION
  (Optional) Map of maps to SSM parameter paths used when templating feeder YAML definitions prior to their conversion to HCL. This variable works identically to `cirrus_feeder_definitions_variables` but sources values from AWS Systems Manager Parameter Store instead of static configuration.
  ...
  **Important**: the targeted SSM parameter must be a simple `String` type. Do **not** use this for passing sensitive information - that is not the intended purpose of this. Values will be stored in plaintext in the state file and visible in Terraform output.
  ...
  The suggested (but not required) structure of this map is to group SSM parameter mappings by their feeder name:
  ```
  {
    example-feeder-1 = {
      image_tag = "/cirrus/feeders/example-feeder-1/latest-image-tag"
    }
    example-feeder-2 = {
      image_tag = "/cirrus/feeders/example-feeder-2/latest-image-tag"
    }
  ...
  Your feeder YAML definitions would leverage this templating using the same attribute lookup syntax as static variables:
  ```
  name: example-feeder-2
  lambda:
    ecr_image_uri: your-ECR-URI:\$${example-feeder-2.image_tag}
  ...
  SSM parameter values are resolved at Terraform plan time, so the current parameter values will be visible in the plan output. If both `cirrus_feeder_definitions_variables` and this variable contain the same feeder/key combination, the SSM value takes precedence. Avoid using duplicate static and SSM variable keys, though, for better readability.
