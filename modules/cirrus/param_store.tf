...
resource "aws_ssm_parameter" "deployment_pointer" {
  name  = "${pointer_prefix}${deployment_name}"
  type  = "String"
  value = jsonencode({
    type = "parameter_store"
    value = "${deployment_prefix}${deployment_name}"
    }
  )
}

resource "aws_ssm_parameter" "event_db_and_table" {
    name = "${deployment_prefix}${deployment_name}/CIRRUS_EVENT_DB_AND_TABLE"
    type = "String"
