...
locals {
  ...
  # Distinct ensures each ARN is only used once.
  ...
  workflow_tasks_batch_job_resources = [
    for job_def in regexall(
      "arn:aws:batch:[a-z0-9-]+:[0-9]{12}:job-definition/[a-zA-Z0-9_-]+",
      local.workflow_state_machine_json
    ) : format("%s:*", job_def)
  ]
  workflow_tasks_batch_queue_resources = regexall(
    "arn:aws:batch:[a-z0-9-]+:[0-9]{12}:job-queue/[a-zA-Z0-9_-]+",
    local.workflow_state_machine_json
  )
  workflow_tasks_batch_resources = distinct(concat(
