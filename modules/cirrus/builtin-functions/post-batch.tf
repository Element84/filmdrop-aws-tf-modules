...
resource "aws_cloudwatch_metric_alarm" "cirrus_post_batch_lambda_errors_warning_alarm" {
  ...
  alarm_name                = "WARNING: ${var.cirrus_prefix}-post-batch Lambda Errors Warning Alarm"
  ...
  alarm_description         = "${var.cirrus_prefix}-post-batch Cirrus Update-State Lambda Errors Warning Alarm"
...
resource "aws_cloudwatch_metric_alarm" "cirrus_post_batch_lambda_errors_critical_alarm" {
  count                     = var.deploy_alarms ? 1 : 0
  alarm_name                = "CRITICAL: ${var.cirrus_prefix}-post-batch Lambda Errors Critical Alarm"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = 5
  metric_name               = "Errors"
  namespace                 = "AWS/Lambda"
  ...
  threshold                 = 100
  treat_missing_data        = "notBreaching"
  alarm_description         = "${var.cirrus_prefix}-post-batch Cirrus Update-State Lambda Errors Critical Alarm"
