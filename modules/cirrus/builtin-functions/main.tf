...
resource "null_resource" "get_cirrus_lambda" {
  ...
  provisioner "local-exec" {
    command = "curl -s -L -o ${local.cirrus_lambda_filename} --fail https://github.com/cirrus-geo/cirrus-geo/releases/download/v${var.cirrus_lambda_version}/cirrus-lambda-dist_v${var.cirrus_lambda_version}_python-${local.cirrus_lambda_pyversion}_aarch64.zip"
  }
}
