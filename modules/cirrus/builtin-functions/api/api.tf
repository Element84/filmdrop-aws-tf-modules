locals {
  ...
  is_private_endpoint = var.api_settings.gateway_rest_type == "PRIVATE" ? true : false
...
resource "aws_lambda_function" "cirrus_api" {
  ...
  timeout          = var.api_settings.lbd_timeout
  memory_size      = var.api_settings.lbd_memory
  publish          = true
  architectures    = ["arm64"]
...
resource "aws_lambda_provisioned_concurrency_config" "cirrus_api_provisioned_concurrency" {
  count = var.api_settings.lbd_provisioned_concurrency > 0 ? 1 : 0

  function_name                     = aws_lambda_function.cirrus_api.function_name
  provisioned_concurrent_executions = var.api_settings.lbd_provisioned_concurrency
...
resource "aws_api_gateway_rest_api" "cirrus_api_gateway" {
  ...
  endpoint_configuration {
    types            = [var.api_settings.gateway_rest_type]
