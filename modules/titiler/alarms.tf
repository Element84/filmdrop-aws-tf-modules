...
resource "aws_cloudwatch_metric_alarm" "warning_titiler_invocations" {
  ...
  dimensions = {
    FunctionName = aws_lambda_function.titiler-lambda.function_name
  }
  alarm_actions             = []
  ok_actions                = []
  ...
  depends_on = [
    aws_lambda_function.titiler-lambda
  ]
}
...
resource "aws_cloudwatch_metric_alarm" "critical_titiler_errors" {
  ...
  alarm_description   = "(Critical) Number of Filmdrop TiTiler Errors has surpassed the Critical threshold"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 5
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
