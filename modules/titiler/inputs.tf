...
variable "titiler_release_tag" {
  description = "Git release tag for: https://github.com/Element84/filmdrop-titiler/releases"
  type        = string
}
...
variable "waf_allowed_url" {
  description = "The url query param in titiler GET requests, and the stac_api_root in POST requests, that should be allowed by WAF rules.  Setting to null will disable blocking behavior and set rules to count instead"
  type        = string
  default     = null
}
