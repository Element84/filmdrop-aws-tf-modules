...
def lambda_handler(event, context):
    ...
    # Debug / Test
    logger.info(f'bbox: {str(min_lat)}, {str(min_long)}, {str(max_lat)}, {str(max_long)}')
    ...
    return {
        "statusCode": 200,
        "body": json.dumps("Success: collections created; siphoning data (if applicable)"),
    }
...
def wait_for_new_collections(collections, stac_dest_url):
    while True:
        ...
        for c in collections:
            ...
            # Debug / Test
            logger.info(f'Collection status code: {str(c["id"])}, {str(response.status_code)}')
