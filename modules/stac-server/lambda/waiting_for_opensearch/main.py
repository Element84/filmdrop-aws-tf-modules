...
def wait_for_opensearch_serverless_collection(collection):
  ...
  while not collection_active:
    ...
    try:
      ...
      logger.info(f'Permissions for {collection} or OpenSearch Serverless may not be available yet, sleeping...: {e}')
      collection_list = {}
    if 'collectionSummaries' in collection_list and len(collection_list['collectionSummaries']) > 0 and collection_list['collectionSummaries'][0]['status'] == 'ACTIVE':
      logger.info(f'Permissions available for {collection}; found active opensearch serverless collection: {collection_list}')
      collection_active = True
    elif 'collectionSummaries' in collection_list and len(collection_list['collectionSummaries']) > 0 and collection_list['collectionSummaries'][0]['status'] == 'FAILED':
      raise Exception(f'Opensearch serverless collection {collection_list} has failed to create...')
    else:
