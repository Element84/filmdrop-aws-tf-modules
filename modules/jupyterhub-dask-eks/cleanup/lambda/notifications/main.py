...
def lambda_handler(event, context):
    ...
    if total_nodes >= int(ANALYTICS_NODE_LIMIT):
        print("Total analytics nodes have exceeded the limit of %s nodes..." % ANALYTICS_NODE_LIMIT)
        print("Sending alert to SNS Topic: ", SNS_TOPIC_ARN)
        message = "The %s account has a total number of %s filmdrop analytics nodes running and has reached or exceeded the limit of %s nodes running. Check with your team to make sure there is no workload running. For clean-up, connect to your %s environment and run the lambda cleanup function." % (STAGE, str(total_nodes), ANALYTICS_NODE_LIMIT, STAGE)
