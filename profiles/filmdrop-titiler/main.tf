module "filmdrop_titiler" {
  ...
  titiler_release_tag                       = var.titiler_inputs.version
  authorized_s3_arns                        = var.titiler_inputs.authorized_s3_arns
  waf_allowed_url                           = var.titiler_inputs.is_private_endpoint ? "" : var.titiler_inputs.titiler_waf_allowed_url == "" ? var.stac_url : var.titiler_inputs.titiler_waf_allowed_url
  request_host_header_override              = var.titiler_inputs.is_private_endpoint ? "" : var.titiler_inputs.titiler_host_header
  ...
  custom_vpce_id                            = var.titiler_inputs.custom_vpce_id
  allowed_extensions_enabled                = var.titiler_inputs.allowed_extensions_enabled
}
...
module "cloudfront_api_gateway_endpoint" {
  ...
  api_gateway_dns_name         = module.titiler.titiler_api_gateway_endpoint
  api_gateway_path             = ""
  web_acl_id                   = module.titiler.titiler_wafv2_web_acl_arn
  project_name                 = var.project_name
  environment                  = var.environment
