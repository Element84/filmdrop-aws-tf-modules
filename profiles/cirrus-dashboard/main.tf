...
module "cloudfront_s3_website" {
  ...
  domain_name                  = var.cirrus_dashboard_inputs.deploy_s3_bucket == false ? var.cirrus_dashboard_inputs.external_content_bucket.external_content_bucket_regional_domain_name : ""
...
module "content_website" {
  count  = var.cirrus_dashboard_inputs.deploy_s3_bucket == true && var.cirrus_dashboard_inputs.deploy_cloudfront == false ? 1 : 0
